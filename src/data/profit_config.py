...
def compute_profit_inputs(df: pd.DataFrame) -> Tuple[float, float, float, float]:
...
def update_profit_config(config: dict, values: Tuple[float, float, float, float]) -> dict:
    avg_loan_amount, avg_interest_rate, avg_loan_term_years, loss_given_default = values
